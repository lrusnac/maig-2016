...
/**
 * The implementation of Ms Pac-Man. This class contains the game engine and all methods required to query the state of the game.
 * First, the mazes are loaded once only as they are immutable. The game then proceeds to initialise all variables using default
 * values. The game class also provides numerous methods to extract the game state as a string (used for replays and for
 * communication via pipes during the competition) and to create copies. Care has been taken to implement the game efficiently to
 * ensure that copies can be created quickly.
 * 
 * The game has a central update method called advanceGame which takes a move for Ms Pac-Man and up to 4 moves for the ghosts. It
 * then updates the positions of all characters, check whether pills or power pills have been eaten and updates the game state
 * accordingly.
 * 
 * All other methods are to access the gamestate and to compute numerous aspects such as directions to taken given a target or a
 * shortest path from a to b. All shortest path distances from any node to any other node are pre-computed and loaded from file.
 * This makes these methods more efficient. Note about the ghosts: ghosts are not allowed to reverse. Hence it is not possible to
 * simply look up the shortest path distance. Instead, one can approximate this greedily or use A* to compute it properly. The
 * former is somewhat quicker and has a low error rate. The latter takes a bit longer but is absolutely accurate. We use the
 * pre-computed shortest path distances as admissable heuristic so it is very efficient.
 */
public final class Game {
    ...
    /**
     * Instantiates a new game. The seed is used to initialise the pseudo-random number generator. This way, a game may be
     * replicated exactly by using identical seeds. Note: in the competition, the games received from the game server are using
     * different seeds. Otherwise global reversal events would be predictable.
     *
     * @param seed
    ...
    /**
     * Gets the game state as a string: all variables are written to a string in a pre-determined order. The string may later be
     * used to recreate a game state using the setGameState() method.
     *
    ...
    /**
     * Sets the game state from a string: the inverse of getGameState(). It reconstructs all the game's variables from the string.
     *
     * @param gameState
    ...
    public void setGameState(String gameState) {
        ...
        pacman = new PacMan(Integer.parseInt(values[index++]), MOVE.valueOf(values[index++]), Integer.parseInt(values[index++]),
    ...
    /**
     * Returns an exact copy of the game. This may be used for forward searches such as minimax. The copying is relatively
     * efficient.
     *
     * @return the game
     */
    ...
    /**
     * Central method that advances the game state using the moves supplied by the controllers. It first updates Ms Pac-Man, then
     * the ghosts and then the general game logic.
     *
     * @param pacManMove
    ...
    /**
     * Updates the game once the individual characters have been updated: check if anyone can eat anyone else. Then update the lair
     * times and check if Ms Pac-Man should be awarded the extra live. Then update the time and see if the level or game is over.
     */
    ...
    /**
     * Checks whether the game is over or not: all lives are lost or 16 levels have been played. The variable is set by the methods
     * _feast() and _checkLevelState().
     *
    ...
    /**
     * Returns the pill index of the node specified. This can be -1 if there is no pill at the specified node.
     * 
     * @param nodeIndex
    ...
    /**
     * Returns the power pill index of the node specified. This can be -1 if there is no power pill at the specified node.
     * 
     * @param nodeIndex
     *            The Index of the node.
    ...
    /**
     * Gets the neighbouring nodes from the current node index excluding the node that corresponds to the opposite of the last move
     * made which is given as an argument.
     *
     * @param nodeIndex
     *            The current node index
     * @param lastModeMade
    ...
    /**
     * Given a node index and a move to be made, it returns the node index the move takes one to. If there is no neighbour in that
     * direction, the method returns -1.
     * 
     * @param nodeIndex
     *            The current node index
     * @param moveToBeMade
    ...
    /**
     * Method that returns the direction to take given a node index and an index of a neighbouring node. Returns null if the
     ...
     *            The last move made
     * @param distanceMeasure
     *            The distance measure required (Manhattan, Euclidean or Straight line)
    ...
    /**
     * Gets the approximate shortest path taking into account the last move made (i.e., no reversals). This is approximate only as
     * the path is computed greedily. A more accurate path can be obtained using A* which is slightly more costly.
     *
     * @param fromNodeIndex
     *            The node index from where to start (i.e., current position)
     * @param toNodeIndex
    ...
    /**
     * Gets the shortest path taking into account the last move made (i.e., no reversals). This is approximate only as the path is
     * computed greedily. A more accurate path can be obtained using A* which is slightly more costly.
     *
     * @param fromNodeIndex
     *            The node index from where to start (i.e., current position)
     * @param toNodeIndex
    ...
    /**
     * Similar to getApproximateShortestPath but returns the distance of the path only. It is slightly more efficient.
     * 
     * @param fromNodeIndex
     *            The node index from where to start (i.e., current position)
     * @param toNodeIndex
    ...
    /**
     * Similar to getShortestPath but returns the distance of the path only. It is slightly more efficient.
     * 
     * @param fromNodeIndex
     *            The node index from where to start (i.e., current position)
     * @param toNodeIndex
