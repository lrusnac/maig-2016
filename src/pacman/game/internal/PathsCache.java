...
public class PathsCache {
    ...
    public int[] getPathFromA2B(int a, int b, MOVE lastMoveMade) {
        ...
        MOVE moveEnteredJunction = fromJunction.lastMove.equals(MOVE.NEUTRAL) ? lastMoveMade : fromJunction.lastMove; // if we are
                                                                                                                      // at a
                                                                                                                      // junction,
                                                                                                                      // consider
                                                                                                                      // last move
            ...
            if (junctionFromId == junctionToId) {
                ...
                for (MOVE move : set) {
                    if (!move.opposite().equals(moveEnteredJunction) && !move.equals(MOVE.NEUTRAL)) {
                        ...
                        if (path.length + junctionsTo.get(q).path.length < minDist)// need to take distance from toJunction to
                                                                                   // target into account
                        {
