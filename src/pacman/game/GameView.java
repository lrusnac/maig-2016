...
/**
 * This class is the view that displays the game. The only thing contestants might need to know about are the helper methods that
 * allow one to add colours and lines to the game view. This could be useful for debugging.
 */
...
public final class GameView extends JComponent {
     ...
     *            Colour to be used
     * @param nodeIndices
     *            The node indices to be highlighted by the chosen colour
     */
     ...
     *            The node indices where the lines start
     * @param toNodeIndices
     *            The node indices where the lines end
     */
    public synchronized static void addLines(Game game, Color color, int[] fromNnodeIndices, int[] toNodeIndices) {
        if (isVisible)
            for (int i = 0; i < fromNnodeIndices.length; i++)
                debugLines.add(new DebugLine(game.getNodeXCood(fromNnodeIndices[i]), game.getNodeYCood(fromNnodeIndices[i]),
    ...
    private void drawPills() {
        ...
        for (int i = 0; i < pillIndices.length; i++)
            if (game.isPillStillAvailable(i))
                bufferGraphics.fillOval(game.getNodeXCood(pillIndices[i]) * MAG + 4, game.getNodeYCood(pillIndices[i]) * MAG + 8, 3,
                        3);
    }
    ...
    public class Images {
        ...
        public Images() {
            ...
            pacman.put(MOVE.UP, new BufferedImage[] { _loadImage("mspacman-up-normal.png"), _loadImage("mspacman-up-open.png"),
